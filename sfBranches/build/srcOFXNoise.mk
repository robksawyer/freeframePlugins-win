...
# 2. Find and Replace the string "OFXNOISE" with a name that describes the source folder you are adding.
...
#     #INCLUDE_DIRS += -I"${OFXNOISE_SRC_DIR}"
...
OFXNOISE_SRC_DIR = ${OF_ROOT}/addons/ofxNoise/src
...
OFXNOISE_OBJ_BASE_DIR := ${OFXNOISE_SRC_DIR}/${shell basename ${OFXNOISE_SRC_DIR}}_obj
...
# folders.
OFXNOISE_OBJ_DIR = ${OFXNOISE_OBJ_BASE_DIR}/${BUILD_CONFIGURATION_NAME}

# Get a list of all .cpp files in the source folder. Only modify this if you want to exclude some files.
OFXNOISE_SRCS = $(shell find ${OFXNOISE_SRC_DIR} -name "*.cpp")

# Create .d and .o file names from all CPP_SRCS files. Only modify this if you want to exclude some files.
OFXNOISE_OBJS = $(shell echo ${OFXNOISE_SRCS} | sed s:${OFXNOISE_SRC_DIR}:${OFXNOISE_OBJ_DIR}:g | sed s:.cpp:.o:g )
...
OBJS_BASE_DIRS += ${OFXNOISE_OBJ_BASE_DIR}
OBJS_DIRS += ${OFXNOISE_OBJ_DIR}
...
OFXNOISE_SRC_DEFINES = ${DEFINES}
OFXNOISE_SRC_COMPILE_FLAGS = ${COMPILE_FLAGS}
...
${OFXNOISE_OBJ_DIR}/%.o: ${OFXNOISE_SRC_DIR}/%.cpp
	@echo 'Building file: $<'
	@echo '------------------------------'
	${MKDIR} ${OFXNOISE_OBJ_DIR}
	${CC} ${OFXNOISE_SRC_DEFINES} ${INCLUDE_DIRS} ${OFXNOISE_SRC_COMPILE_FLAGS} -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -o"$@" "$<"
	@echo '------------------------------'
