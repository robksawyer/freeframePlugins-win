...
PROJECT_OBJ_BASE_DIR := ${PROJECT_SRC_DIR}/../${shell basename ${PROJECT_SRC_DIR}}_obj
...
# folders.
PROJECT_OBJ_DIR = ${PROJECT_OBJ_BASE_DIR}/${BUILD_CONFIGURATION_NAME}

# Get a list of all .cpp files in the source folder. Only modify this if you want to exclude some files.
PROJECT_SRCS = $(shell find ${PROJECT_SRC_DIR} -name "*.cpp")

# Create .d and .o file names from all CPP_SRCS files. Only modify this if you want to exclude some files.
PROJECT_OBJS = $(shell echo ${PROJECT_SRCS} | sed s:${PROJECT_SRC_DIR}:${PROJECT_OBJ_DIR}:g | sed s:.cpp:.o:g )
...
OBJS_BASE_DIRS += ${PROJECT_OBJ_BASE_DIR}
OBJS_DIRS += ${PROJECT_OBJ_DIR}
...
PROJECT_SRC_DEFINES = ${DEFINES}
PROJECT_SRC_COMPILE_FLAGS = ${COMPILE_FLAGS}
...
${PROJECT_OBJ_DIR}/%.o: ${PROJECT_SRC_DIR}/%.cpp
	...
	${CC} ${PROJECT_SRC_DEFINES} ${INCLUDE_DIRS} ${PROJECT_SRC_COMPILE_FLAGS} -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -o"$@" "$<"
	@echo '------------------------------'
