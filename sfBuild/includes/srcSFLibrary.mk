...
# 2. Find and Replace the string "SFLIBRARY" with a name that describes the source folder you are adding.
...
SFLIBRARY_SRC_DIR = ../../sfLibrary/src

INCLUDE_DIRS += -I"${SFLIBRARY_SRC_DIR}"
...
SFLIBRARY_OBJ_BASE_DIR := ${SFLIBRARY_SRC_DIR}/${shell basename ${SFLIBRARY_SRC_DIR}}_obj
...
# folders.
SFLIBRARY_OBJ_DIR = ${SFLIBRARY_OBJ_BASE_DIR}/${BUILD_CONFIGURATION_NAME}

# Get a list of all .cpp files in the source folder. Only modify this if you want to exclude some files.
SFLIBRARY_SRCS = $(shell find ${SFLIBRARY_SRC_DIR} -name "*.cpp")

# Create .d and .o file names from all CPP_SRCS files. Only modify this if you want to exclude some files.
SFLIBRARY_OBJS = $(shell echo ${SFLIBRARY_SRCS} | sed s:${SFLIBRARY_SRC_DIR}:${SFLIBRARY_OBJ_DIR}:g | sed s:.cpp:.o:g )
...
OBJS_BASE_DIRS += ${SFLIBRARY_OBJ_BASE_DIR}
OBJS_DIRS += ${SFLIBRARY_OBJ_DIR}
...
SFLIBRARY_SRC_DEFINES = ${DEFINES}
SFLIBRARY_SRC_COMPILE_FLAGS = ${COMPILE_FLAGS}
...
${SFLIBRARY_OBJ_DIR}/%.o: ${SFLIBRARY_SRC_DIR}/%.cpp
	@echo 'Building file: $<'
	@echo '------------------------------'
	${MKDIR} ${SFLIBRARY_OBJ_DIR}
	${CC} ${SFLIBRARY_SRC_DEFINES} ${INCLUDE_DIRS} ${SFLIBRARY_SRC_COMPILE_FLAGS} -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -o"$@" "$<"
	@echo '------------------------------'
